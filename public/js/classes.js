  ...
  const updateSpecs = async () => {
    try {
      ...
      devicesList.innerHTML = `
                ...
                <p><strong>USB Devices:</strong> ${specs.connectedDevices.usb
                  .map(
                    (device) =>
                      `<strong>${device.type}:</strong> (${device.name})`
                  )
                  .join(", ")}</p>
                <p><strong>Bluetooth Devices:</strong> ${specs.connectedDevices.bluetooth
                  .map((device) => device.name)
                  .join(", ")}</p>
                <p><strong>Printers:</strong> ${specs.connectedDevices.printers
...
document
  .getElementById("computerRegistrationForm")
